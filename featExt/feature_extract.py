    ...
    while True:
        ...
        if not data_msg_queue.empty():
            ...
            if sum(flag) == 2:
                ...
                ds = np.concatenate((ds_s1[:com_len], ds_s2[:com_len]), axis=0)
                sensor_time = str(max(s1_time, s2_time))
                payload1={"type": "data", "contents": {"down_sample": ds, "time":sensor_time}}
                payload2={"type": "data", "contents": {"features": feat_array, "time":sensor_time}}
                feat = [0, 0]
                flag = [0, 0]
                try:
                    # ecci_client.publish(payload1, "save-1")
                    ecci_client.publish(payload2, ["irons-1", "life-1", "running-1"])
